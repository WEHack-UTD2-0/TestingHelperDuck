        ...
        try:
            ...
            if ctx.user.get_role(config['MENTOR_ROLE_ID']) is None:
                ...
                ticket_info_query = db_cursor.execute(
                    ...
                    {'ticket_id': ticket_id, 'user_id': ctx.user.id}).fetchone()
                ...
                if claimed == 1:
                    await ctx.send(f'Mentor {mentor_name} has claimed this ticket. Please contact them to close it.',
                                   ephemeral=True)
            ...
            db_cursor.execute('UPDATE mentors SET tickets_closed = tickets_closed + 1 WHERE id = :mentor_id',
...
async def claim(ctx: nc.Interaction, ticket_id: int) -> None:
    with db_connection:
        ...
        try:
            ...
            mentor_query = db_cursor.execute('SELECT 1 FROM mentors WHERE id = :mentor_id',
                                             {'mentor_id': ctx.user.id}).fetchone()
            ...
            db_cursor.execute('UPDATE mentors SET tickets_claimed = tickets_claimed + 1 WHERE id = :mentor_id',
                              {'mentor_id': ctx.user.id})
            ...
            db_cursor.execute('UPDATE tickets SET help_thread_id = :help_thread_id WHERE id = :ticket_id',
...
async def helpme(ctx: nc.Interaction, author_location: str, ticket_message: str) -> None:
    with db_connection:
        ...
        try:
            ...
            ticket_embed = nc.Embed(title='New Ticket Opened! :tickets:',
        ...
        try:
            ...
            if ctx.user.get_role(config['MENTOR_ROLE_ID']) is not None:
                tickets_query = db_cursor.execute(
                    'SELECT id, closed FROM tickets WHERE mentor_assigned_id = :mentor_id',
                    {'mentor_id': ctx.user.id}).fetchall()

                if tickets_query == []:
                    await ctx.send('You have not claimed any tickets! Use `/opentix` view open tickets to claim.',
                                   ephemeral=True)
                ...
                tickets_embed = nc.Embed(title='Your Claimed Tickets :sunglasses:',
                ...
                # this is an iterable
                tickets_query = db_cursor.execute(
                    'SELECT id, claimed, closed FROM tickets WHERE author_id = :author_id',
                    {'author_id': ctx.user.id}).fetchall()
                ...
                tickets_embed = nc.Embed(title='Your Tickets :man_dancing:',
                                         description='Use `/status` with the ticket ID for more information on a given ticket.')
...
async def status(ctx: nc.Interaction, ticket_id: int) -> None:
    with db_connection:
        ...
        try:
            ...
            if ctx.user.get_role(config['ORGANIZER_ROLE_ID']) is not None or ctx.user.get_role(
                    config['MENTOR_ROLE_ID']) is not None:
                ...
                ticket_query = db_cursor.execute(
                    ...
                    {'author_id': ctx.user.id, 'ticket_id': ticket_id}).fetchone()

            if ticket_query is None:
                logging.warning(
                    f'User {user_name} tried to view a ticket with ID {ticket_id}, but database query returned nothing.')

                await ctx.send(f'You don\'t have ownership over a ticket with ID {ticket_id}. Maybe you made a typo?',
                               ephemeral=True)
            ...
            ticket_embed.add_field(name='__Claimed__ :face_with_monocle:',
                                   value=(':white_check_mark:' if claimed == 1 else ':no_entry:'))
            ticket_embed.add_field(name='__Closed__ :tada:',
                                   value=(':white_check_mark:' if closed == 1 else ':no_entry:'))
...
@nc_app_checks.check(lambda ctx: ctx.user.get_role(config['MENTOR_ROLE_ID']) is not None or ctx.user.get_role(
    config['ORGANIZER_ROLE_ID']) is not None)
async def opentix(ctx: nc.Interaction) -> None:
    with db_connection:
        ...
        try:
            ...
            # this is an iterable
            tickets_query = db_cursor.execute(
                'SELECT id, author_location, author, message FROM tickets WHERE claimed = 0 AND closed = 0').fetchall()
            ...
            # prep the data for embed.
            ticket_ids = map(str, ticket_ids)
            logistics = map((lambda x: f'{x[0]} @ *{x[1][:10] + "..." if len(x[1]) > 10 else x[1]}*'),
                            zip(authors, locations))
            messages = map((lambda x: f'{x[:10]}...' if len(x) > 10 else x), messages)

            tickets_embed = nc.Embed(title='Open Tickets :dancer:',
...
async def alltix(ctx: nc.Interaction) -> None:
    with db_connection:
        ...
        try:
            ...
            tickets_embed = nc.Embed(title='All Tickets :face_with_spiral_eyes:',
    ...
    with db_connection:
        ...
        try:
            ...
            mentors_query = db_cursor.execute(
                'SELECT name, tickets_claimed, tickets_closed FROM mentors ORDER BY tickets_closed DESC').fetchall()
            ...
            tickets_embed = nc.Embed(title='Mentor Leaderboard :fire:',
                                     description='Whoever closes the most tickets wins!')
